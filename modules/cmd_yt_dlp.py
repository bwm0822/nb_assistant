...
def search(query):
    # 設定 yt-dlp 參數
    if query == '': return {"status":-1, "text":"搜尋關鍵字為空"}
    ...
    index = 0  # 重設索引
    if not videos:
        return {"status":-1, "text":"找不到影片"}  # 回傳搜尋結果
    ...
    return {"status":1, "playlist":playlist}  # 回傳搜尋結果

def play(video):
    # 只想播放 單一影片或音訊，不需要進階控制
    global player
    if video == '': return {"status":-1, "text":"video 為空"}
    ...
    player.play()
    return {"status":1, "text":f"播放：{playlist[index]}"}
...
def pause():
    if player is None: return {"status":-1, "text":"播放器未開啟"}
    player.pause()
...
def close():
    ...
    player.release()  # 釋放資源
    player = None
    return {"status":1, "text":"播放器已關閉"}

def previous():
    # 這裡可以實現播放上一首音樂的邏輯
    global index
    if player is None: return {"status":-1, "text":"播放器未開啟"}
    if len(playlist) == 0: return {"status":-1, "text":"播放清單為空"}
...
def forward(sceonds):
    ...
    return {"status":1, "text":f"快轉 {seconds}秒"}
...
def backward(sceonds):
    ...
    print(f"當前時間：{current_time}秒")
    player.set_time(int((current_time - seconds) * 1000))  # 設定新的播放時間
    return {"status":1, "text":f"倒退 {seconds}秒"}
...
def volumeup(delta=20):
    ...
    return {"status":1, "text":f"音量增加到 {new_volume}%"}

def volumedown(delta=20):
    # 將音量減少 20%
    if player is None: return {"status":-1, "text":"播放器未開啟"}
    current_volume = player.audio_get_volume()  # 取得當前音量
    new_volume = max(current_volume - delta, 0)  # 限制在 0-100 範圍內
    player.audio_set_volume(new_volume)  # 設定新的音量
    return {"status":1, "text":f"音量減少到 {new_volume}%"}
...
def volume(vol):
    ...
    return {"status":1, "text":f"音量設為 {vol}%"}
